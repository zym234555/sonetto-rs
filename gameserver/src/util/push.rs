...
pub async fn send_red_dot_push(
    ...
    let red_dot_records = {
        ...
        match define_ids {
            Some(ids) => {
                red_dots::red_dots::get_red_dots_by_defines(&conn.state.db, user_id, &ids).await?
            }
            None => red_dots::red_dots::get_red_dots(&conn.state.db, user_id).await?,
        }
    };
    ...
    if !red_dot_records.is_empty() {
        ...
        conn.notify(CmdId::UpdateRedDotPushCmd, push).await?;
    }
...
pub async fn send_material_change_push(
    ...
    conn.notify(CmdId::MaterialChangePushCmd, push.clone())
        .await?;
...
pub async fn send_dungeon_update_push(
    ...
    let mut conn = ctx.lock().await;
    conn.notify(CmdId::DungeonUpdatePushCmd, push).await?;
...
pub async fn on_finish_story_notify(
    ctx: Arc<Mutex<ConnectionContext>>,
    user_id: i64,
    story_id: i32,
) -> Result<(), AppError> {
    let sid: Option<i32> = Some(story_id);
    let notify = StoryFinishPush { story_id: sid };
    ...
    finish_story(&pool, user_id, story_id).await?;
