...
pub async fn on_get_dungeon(
    ...
    let (
        ...
        tokio::try_join!(
            dungeons::get_dungeon_last_hero_groups(&conn.state.db, player_id),
            dungeons::get_unlocked_maps(&conn.state.db, player_id),
            dungeons::get_elements(&conn.state.db, player_id),
            dungeons::get_reward_points(&conn.state.db, player_id),
            dungeons::get_equip_sp_chapters(&conn.state.db, player_id),
            dungeons::get_chapter_type_nums(&conn.state.db, player_id),
            dungeons::get_finished_elements(&conn.state.db, player_id),
            dungeons::get_finished_puzzles(&conn.state.db, player_id),
        )?
    };
...

    {
        ...
        conn.send_reply(CmdId::GetDungeonCmd, reply, 0, req.up_tag)
            .await?;
    }
...
async fn send_dungeon_info_pushes(
    ...
    let dungeon_chunks = {
        let conn = ctx.lock().await;
        database::db::game::dungeons::get_user_dungeons_chunked(&conn.state.db, user_id).await?
    };
